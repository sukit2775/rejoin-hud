...
-- Notification function (เวอร์ชันปรับปรุง: เพิ่มไอคอน + ปุ่มกดเปิด-ปิดหน้าจอ)
local function showCustomNotif(title, text, iconId)
    -- ลบอันเก่าออกก่อนหากมีอยู่
    local oldNotif = PlayerGui:FindFirstChild("CustomNotification")
    if oldNotif then oldNotif:Destroy() end
    ...
    -- 1. เพิ่มไอคอน (ImageLabel)
    local icon = Instance.new("ImageLabel")
    icon.Name = "Icon"
    icon.Size = UDim2.new(0, 40, 0, 40)
    icon.Position = UDim2.new(0, 10, 0.5, -20)
    icon.BackgroundTransparency = 1
    -- หากไม่ได้ใส่ iconId จะใช้ไอคอนเริ่มต้นให้
    icon.Image = iconId or "https://i.ibb.co/bgQ9GgKv/icon-SUKITHUB.jpg" 
    icon.Parent = frame
    ...
    -- ข้อความรายละเอียด (Text Label)
    local textLabel = Instance.new("TextLabel")
    textLabel.Size = UDim2.new(1, -65, 0.5, -5)
    textLabel.Position = UDim2.new(0, 60, 0.5, 0)
    textLabel.BackgroundTransparency = 1
    ...
    -- 2. ปุ่มสำหรับ เปิด-ปิด จอแสดงผล (Toggle Button)
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "ToggleButton"
    toggleBtn.Size = UDim2.new(0, 80, 0, 30)
    toggleBtn.Position = UDim2.new(0, 10, 0, 10) -- วางไว้ที่มุมซ้ายบนของหน้าจอ
    ...
    -- สลับเปิด / ปิด การมองเห็นของกรอบแจ้งเตือนเมื่อคลิกปุ่ม
...
-- การเรียกใช้งาน (สามารถใส่ Asset ID ของรูปภาพเป็นพารามิเตอร์ตัวที่ 3 ได้)
